...
ArcFaceDetection::~ArcFaceDetection()
{
	...
	multiFaceInfoMemoryRelease(&m_videoFaceInfo);
}
...
void ArcFaceDetection::initData()
{
	...
	multiFaceInfoMemoryAllocation(&m_videoFaceInfo);
	...
	if (MOK == res || MERR_ASF_ALREADY_ACTIVATED == res)
	{
		...
		MInt32 imageMask = ASF_FACE_DETECT | ASF_FACERECOGNITION | ASF_IMAGEQUALITY | ASF_LIVENESS | ASF_IR_LIVENESS |
			ASF_MASKDETECT | ASF_AGE | ASF_GENDER;
		...
		MInt32 videoFtMask = ASF_FACE_DETECT;
...
inline void myCvPutText(IplImage* img, QString str, ASF_MultiFaceInfo multiFaceInfo, 
	CvFont font, bool isSucessed, int multiple)
{
	cvPutText(img, str.toStdString().c_str(),
		cvPoint(multiFaceInfo.faceRect[0].left, multiFaceInfo.faceRect[0].bottom + FACE_ATTRIBUTE_HEIGHT * multiple),
		&font, isSucessed ? cvScalar(0, 255, 0) : cvScalar(0, 0, 255));
}
...
void ArcFaceDetection::ftPreviewData()
{
	...
	//人脸属性默认阈值0.5，可根据实际应用场景进行调整
	m_videoFtEngine.SetFaceAttributeParam(EYEOPEN_THRESHOLD, MOUTHCLOSE_THRESHOLD, WEARGLASSES_THRESHOLD);
	...
	while (m_isOpenCamera)
	{
		...
		if (!m_rgbFrame.empty())
		{
			...
			if (MOK == res)
			{
				bool isSucessed = m_isCompareSuccessed && m_isLive;
				//画人脸框
				cvRectangle(rgbImage, cvPoint(m_videoFaceInfo.faceRect[0].left, m_videoFaceInfo.faceRect[0].top),
					...
					isSucessed ? cvScalar(0, 255, 0) : cvScalar(0, 0, 255), 2);
				...
				//画额头区域框
				cvRectangle(rgbImage, cvPoint(m_videoFaceInfo.foreheadRect[0].left, m_videoFaceInfo.foreheadRect[0].top),
					cvPoint(m_videoFaceInfo.foreheadRect[0].right, m_videoFaceInfo.foreheadRect[0].bottom),
					cvScalar(192, 192, 192), 2);

				//绘制人脸属性
				myCvPutText(rgbImage, QString::number(m_videoFaceInfo.faceID[0]), m_videoFaceInfo, font, isSucessed, 1);

				QString strIsWithinBoundary = "FaceIsWithinBoundary:" + QString::number(m_videoFaceInfo.faceIsWithinBoundary[0]);
				myCvPutText(rgbImage, strIsWithinBoundary, m_videoFaceInfo, font, isSucessed, 2);

				QString strWearGlasses = "WearGlasses:" + QString::number(m_videoFaceInfo.faceAttributeInfo.wearGlasses[0]);
				myCvPutText(rgbImage, strWearGlasses, m_videoFaceInfo, font, isSucessed, 3);

				QString strEyeOpen = "EyeOpen:[" + QString::number(m_videoFaceInfo.faceAttributeInfo.leftEyeOpen[0]) + 
					" " + QString::number(m_videoFaceInfo.faceAttributeInfo.rightEyeOpen[0]) + "]";
				myCvPutText(rgbImage, strEyeOpen, m_videoFaceInfo, font, isSucessed, 4);

				QString strMouthClose = "MouthClose:" + QString::number(m_videoFaceInfo.faceAttributeInfo.mouthClose[0]);
				myCvPutText(rgbImage, strMouthClose, m_videoFaceInfo, font, isSucessed, 5);
...
void ArcFaceDetection::frPreviewData()
{
	...
	while (m_isOpenCamera)
	{
		...
		res = m_imageFrEngine.searchFaceFeature(&faceFeature, maxThreshold, maxIndex, m_compareModel);
		...
		if (MOK == res && maxThreshold > compareThreshold->text().toFloat())
		{
...
void ArcFaceDetection::showRegisterThumbnail()
{
	...
	ASF_MultiFaceInfo multiFaceInfo = { 0 };
	multiFaceInfoMemoryAllocation(&multiFaceInfo);
	...
	for (int i = 0; i < picPathList.size(); ++i)
	{
		...
		m_imageFrEngine.registerFace(m_nIndex, &faceFeature);
		SafeFree(faceFeature.feature);
	...
	multiFaceInfoMemoryRelease(&multiFaceInfo);
...
void ArcFaceDetection::detectionRecognitionImage(IplImage* recognitionImage, ASF_SingleFaceInfo& faceInfo, QString& strInfo)
{
	...
	else
	{
		...
		res = m_imageEngine.FaceProcess(multiFaceInfo, recognitionImage, ageInfo, genderInfo, livenessInfo);
		if (MOK == res)
		{
...
void ArcFaceDetection::faceCompare()
{
	...
	MFloat maxThreshold = 0.0;
	MRESULT pairRes = m_imageFrEngine.searchFaceFeature(&m_recognitionImageFeature, maxThreshold, maxIndex, m_compareModel);
	...
	//	}
	//}
	if (MERR_ASF_SEARCH_EMPTY == pairRes) {
		QMessageBox::about(NULL, QStringLiteral("提示"), QStringLiteral("请注册人脸库！"));
		return;
	}
